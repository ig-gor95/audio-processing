...
class OrderPatternsDetector:
    ...
    def __call__(self, texts: pd.DataFrame):
        ...
        return (
            pd.Series(batch_match(texts, self.offer_patterns), index=texts.index),
            pd.Series(batch_match(texts, self.processing_patterns), index=texts.index),
            pd.Series(batch_match(texts, self.resume_patterns), index=texts.index)
        )
